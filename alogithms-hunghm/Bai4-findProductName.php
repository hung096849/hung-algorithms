<?php
 
 function findProduct($nameProduct, $listProduct) {
    foreach ($listProduct as $key => $val) { 
        if ($val['name'] === $nameProduct) { //nếu tham số truyền vào đúng với giá trị có trong mảng, hiển thị ra thông tin product đó
             $val['name'];
             $val['price'];
             $val['quality'];
             $val['categoryId'];
            return $val;
        }
    }
    return null;
 }
   
 $listProduct= array(
     array('name'=>'CPU', 'price'=>750, 'quality'=>10, 'categoryId'=>1),
     array('name'=>'RAM', 'price'=>50, 'quality'=>2, 'categoryId'=>2),
     array('name'=>'HDD', 'price'=>70, 'quality'=>1, 'categoryId'=>2),
     array('name'=>'Main', 'price'=>400, 'quality'=>3, 'categoryId'=>1),
     array('name'=>'Keyboard', 'price'=>30, 'quality'=>8, 'categoryId'=>4),
     array('name'=>'Mouse', 'price'=>25, 'quality'=>10, 'categoryId'=>6),
 );
//hiển thị ra thông tin product cần tìm,vì là mảng nên dùng hàm implode để xuất ra thành chuỗi,dấu phẩy là dùng để ngăn cách kí tự
echo implode(',',$nameProduct= findProduct('RAM',$listProduct));
?>
